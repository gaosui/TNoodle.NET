    ...
    public class CubePuzzle : Puzzle
    {
        ...
        public class CubeState : PuzzleState
        {
            ...
            public string ToFaceCube()
            {
                ...
                return faces.Select(f => _image[(int) f]).Aggregate("", (current2, faceArr) => faceArr.Aggregate(current2, (current1, t) => t.Aggregate(current1, (current, t1) => current + ((Face) t1).ToString())));
            }
