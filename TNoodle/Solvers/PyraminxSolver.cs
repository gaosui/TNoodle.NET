...
namespace TNoodle.Solvers
{
    public class PyraminxSolver
    {
        ...
        /** There are 4 corners on the pyraminx that are in a fixed position.
          ...
          *                         B
          */
        internal const int N_EDGE_PERM = 720; // Number of permutations of edges
        internal const int N_EDGE_ORIENT = 32; // Number of orientations of edges
        internal const int N_CORNER_ORIENT = 81; // Number of orientations of corners
        internal const int N_ORIENT = N_EDGE_ORIENT * N_CORNER_ORIENT;
        internal const int N_TIPS = 81; // Number of tips positions
        internal const int N_MOVES = 8; // Number of moves
        internal const int MAX_LENGTH = 20;
        internal static readonly String[] moveToString = { "U", "U'", "L", "L'", "R", "R'", "B", "B'" };
        internal static readonly String[] inverseMoveToString = { "U'", "U", "L'", "L", "R'", "R", "B'", "B" };
        internal static readonly String[] tipToString = { "u", "u'", "l", "l'", "r", "r'", "b", "b'" };
        internal static readonly String[] inverseTipToString = { "u'", "u", "l'", "l", "r'", "r", "b'", "b" };
        ...
        private static void initPrun()
        {
            ...
            int done = 1;
            for (int length = 0; done < N_EDGE_PERM / 2; length++)
         ...
         */
        private bool search(int edgePerm, int edgeOrient, int cornerOrient, int depth, int length, int last_move, int[] solution, Random randomiseMoves)
        {
             ...
             */
            int randomOffset = randomiseMoves.Next(N_MOVES);
            for (int move = 0; move < N_MOVES; move++)
            {
        ...
        public PyraminxSolverState randomState(Random r)
        {
            ...
            do
            {
                state.edgePerm = r.Next(N_EDGE_PERM);
            } while (prunPerm[state.edgePerm] == -1); // incorrect permutation (bad parity)
            state.edgeOrient = r.Next(N_EDGE_ORIENT);
            state.cornerOrient = r.Next(N_CORNER_ORIENT);
            state.tips = r.Next(N_TIPS);
            return state;
        }
        ...
        private String solve(PyraminxSolverState state, int desiredLength, bool exactLength, bool inverse, bool includingTips)
        {
            Random r = new Random();
